...
functionscall()
{
    ...
    if(getdvar("freezedumbass") == "[ON]")
    self thread freezeloop();
}


togglefreezedumbass()
{
    if(getdvar("freezedumbass") == "[OFF]")
    {
        setdvar("freezedumbass","[ON]");
        self thread freezeloop();
    }
    else
    {
        setdvar("freezedumbass","[OFF]");
        self notify("stopfreezedumbass");
        ents = GetEntArray();
        foreach(ent in ents)
...
alwayscanswapspecificloop()
{
    ...
    while(true)
    {
        ...
        if(x != z && self hasWeapon(z) && !IsSubStr(x,"alt"))
        {
...
givedamoney()
{
    if(self getcoopplayerdata("alienSession", "currency") == 6000)
    {
        self iprintlnbold("Money ^:Full");
        return;
    }
    self setcoopplayerdata("alienSession", "currency", 6000);
	self iprintlnbold("Given ^:$6000");
}

givedapoints()
{
    amount = self getcoopplayerdata("alienSession", "skill_points");
	self setcoopplayerdata("alienSession", "skill_points", amount + 5);
	self iprintlnbold("Given ^:5 ^7Skill Points");
}

givedapointss()
{
    amount = self getcoopplayerdata("alienSession", "skill_points");
	self setcoopplayerdata("alienSession", "skill_points", amount + 100);
	self iprintlnbold("Given ^:5 ^7Skill Points");
}
...
doendgame()
{
    self thread maps\mp\alien\_hud::alienOutcomeNotify( "allies", 1 );
    wait 1;
...
aimbotloop()
{
    ...
    while(true)
    {
        ...
        //i hate nesting shit but fuck it
        foreach(ent in ents)
        {
            if(ent != self && IsDefined(ent.team) && self.team != ent.team && IsAlive(ent))
            {
                if(self GetCurrentWeapon() == getdvar("aimbot_weapon") && Distance(ent.origin, center) < GetDvarInt("aimbot_range"))
                {
                    ent DoDamage( ent.health + 100, ent.origin, self, self );
                    if(getdvar("endgame") == "[ON]")
                    {
