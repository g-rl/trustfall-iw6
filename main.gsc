...
init()
{
    ...
    //nope setdvar("jump_enablefalldamage", 0);
...
OnPlayerSpawned()
{
    while(true)
    {
        ...
        if(self ishost())
        {
            ...
            self thread savepos();
            self thread loadpos();
            self thread givedamoney();
            self thread givedapointss();
            ...
            wait 3;
            self iprintlnbold("^:#campaigncrew");
        }
...
setdvars()
{
    SetDvarIfUni("freezedumbass","[OFF]");
    self.canspawnheli = true;
    self.innoclip = false;
    self.pers["movpos"] = 0;
    SetDvarIfUni("linksjitter","[OFF]");
    SetDvarIfUni("endgame","[OFF]");
